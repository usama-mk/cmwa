...
import { FileText, Upload, Calendar, CheckCircle, Clock, AlertCircle, Save, Check, X } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-4">Loading your projects...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
            {projects.map((project) => (
              <div key={project.id} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-6 transition-colors duration-200">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 space-y-2 sm:space-y-0">
                  ...
                  <div className={`px-3 py-1 rounded-lg border text-sm font-medium flex items-center space-x-1 ${getStatusColor(project.status)} self-start`}>
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 space-y-2 sm:space-y-0">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Google Drive Link</span>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                    <input
                      ...
                      value={editingLinks[project.id] !== undefined ? editingLinks[project.id] : (project.drive_link || '')}
                      onChange={(e) => setEditingLinks(prev => ({
                        ...prev,
                        [project.id]: e.target.value
                      }))}
                      onFocus={() => handleLinkEdit(project.id, project.drive_link || '')}
                    ...
                    {editingLinks[project.id] !== undefined && (
                      <div className="flex space-x-2">
                        ...
                        >
                          ...
                          <span>{updatingLink === project.id ? 'Updating...' : 'Update'}</span>
                        </button>
                      </div>
                    )}
                  </div>
            ...
            {projects.length === 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-8 sm:p-12 text-center transition-colors duration-200">
                ...
                <p className="text-gray-600 dark:text-gray-400">Your projects will appear here once they're created.</p>
              </div>
            )}
          </div>
              ...
              <div className="space-y-4">
                {updates.map((update) => (
                  <div key={update.id} className="border-l-2 border-blue-500 pl-4">
                    <h4 className="font-medium text-gray-900 dark:text-white text-sm sm:text-base">{update.title}</h4>
                    {update.description && (
                      <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">{update.description}</p>
                    )}
                ...
                {updates.length === 0 && (
                  <p className="text-gray-600 dark:text-gray-400 text-sm">No recent updates</p>
                )}
              </div>
            </div>
          </div>
